...
public class CommentDatafetcher {
    ...
    return io.spring.graphql.types.PageInfo.newBuilder()
        ...
        .endCursor(
            comments.getEndCursor() == null
                ? null
                : comments.getEndCursor().toString())
  ...
  private Comment buildCommentResult(CommentData comment) {
    return Comment.newBuilder()
        ...
        .updatedAt(DateTimeFormatter.ISO_OFFSET_DATE_TIME.format(comment.getCreatedAt().atOffset(ZoneOffset.UTC)))
        .createdAt(DateTimeFormatter.ISO_OFFSET_DATE_TIME.format(comment.getCreatedAt().atOffset(ZoneOffset.UTC)))
        .build();
  }
}
